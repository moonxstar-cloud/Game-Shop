...
function Home({ searchQuery }) {
  ...
  // Fetch games based on search query or default pagination
  const fetchGames = async () => {
    let url = `https://api.rawg.io/api/games?key=c542e67aec3a4340908f9de9e86038af&page=${currentPage}&page_size=${pageSize}`;
    
    // If searchQuery is present, add it to the API request
    if (searchQuery) {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["games", currentPage, searchQuery], // Include searchQuery in the query key
    queryFn: fetchGames,
  });
